...
# 电竞馆智能选址系统 - 一键修复脚本
# 修复内容：
#   1. config.py: 连通性测试接口 405 (GET->POST)
#   2. evaluate.py: 路由双重前缀 404
#   3. chat.py: 路由双重前缀 404
# 使用方法：
#   cd /path/to/ai-lvshu-main
#   chmod +x fix_server.sh && sudo ./fix_server.sh
# ============================================================
...
# ---- 修复 1: evaluate.py 去掉双重 prefix ----
echo "[1/3] 修复 evaluate.py 路由前缀..."
sed -i 's/router = APIRouter(prefix="\/evaluate", tags=\["评估"\])/router = APIRouter(tags=["评估"])/' \
  "$BACKEND_DIR/app/api/evaluate.py"
# 验证
if grep -q 'prefix="/evaluate"' "$BACKEND_DIR/app/api/evaluate.py"; then
  echo "  ❌ evaluate.py 修复失败，请手动编辑"
else
  echo "  ✅ evaluate.py 修复成功"
fi

# ---- 修复 2: chat.py 去掉双重 prefix ----
echo "[2/3] 修复 chat.py 路由前缀..."
sed -i 's/router = APIRouter(prefix="\/chat", tags=\["对话评估"\])/router = APIRouter(tags=["对话评估"])/' \
  "$BACKEND_DIR/app/api/chat.py"
if grep -q 'prefix="/chat"' "$BACKEND_DIR/app/api/chat.py"; then
  echo "  ❌ chat.py 修复失败，请手动编辑"
else
  echo "  ✅ chat.py 修复成功"
fi

# ---- 修复 3: config.py 完全替换为新版本 ----
echo "[3/3] 替换 config.py 为新版本..."
cat > "$BACKEND_DIR/app/api/config.py" << 'PYEOF'
"""
系统配置 API
- GET  /api/v1/system/config/       - 获取所有配置（列表格式）
- POST /api/v1/system/config/batch  - 批量更新配置项
...
if command -v supervisorctl &>/dev/null; then
  sudo supervisorctl restart esports-backend && echo "  ✅ Supervisor 重启成功"
else
  echo "  ⚠️  未检测到 supervisor，请手动重启 uvicorn 进程"
  echo "     kill \$(pgrep -f uvicorn) && cd backend && source venv/bin/activate && uvicorn main:app --host 0.0.0.0 --port 8000 &"
fi

echo ""
echo "=== 修复完成 ==="
echo "请刷新浏览器页面，再次测试连通性。"
echo "如仍有问题，请查看日志：sudo tail -f /var/log/esports-backend.out.log"
