...
*   **后端 (Backend)**：Python 3.11 + FastAPI + SQLAlchemy + Pydantic + Server-Sent Events (SSE)。
...
### 阶段 4：Agentic RAG 报告、智能推荐与可视化工作流 (第 4-5 周)
*   **目标**：打通包含多路召回与重排的 Agentic RAG 链路，生成高质量报告并支持宏观区域推荐，**同时实现类 Dify 的工作流实时可视化输出**。
*   **核心任务**：
    ...
    2.  **工作流实时可视化输出（类 Dify 风格）**：
        *   后端通过 **Server-Sent Events (SSE)** 或 WebSocket，将 Agentic RAG 的每一步判断和执行结果实时推送到前端。
        *   前端设计专门的“思考过程”面板（类似 Dify 的调试日志或 ChatGPT 的思考折叠框），展示如下内容：
            *   `[思考] 意图分析：需要检索外部数据`
            *   `[执行] 路由选择：高德 API + pgvector`
            *   `[结果] 高德 API 召回 0 条竞品数据`
            *   `[思考] 扩大搜索半径至 3km，发起第二轮检索...`
            *   `[结果] 重排模型打分完毕，保留 Top 3 证据`
        *   这极大方便了开发者的调试定位，也让用户对系统的评估过程产生信任感。
    3.  **LLM 报告生成**：组装重排后的历史经验、当前得分（基于自适应权重）与用户短期/长期记忆，调用推理模型生成《智能选址评估报告》。
    4.  **智能区域推荐**：前端框选区域，后端网格化评估，推荐 Top 3 坐标，并规避已有连锁店辐射圈。
*   **估算**：8 天。
...
└── backend/                    # FastAPI 后端代码
    ...
    │       └── sse_logger.py   # 工作流实时可视化日志推送服务 (SSE)
└── docs/                       # 项目文档
```
